...
class SynthTiger(templates.Template):
    ...
    def generate(self):
        ...
        fg_image, fg_image2, label, label2 = self._generate_fg(fg_color, fg_style)
        # fg_image2, label2 = self._generate_fg(fg_color, fg_style)
        
        fg_image_shape = fg_image.shape[:2][::-1]
        fg_image_shape2 = fg_image2.shape[:2][::-1]
        
        if fg_image_shape >= fg_image_shape2:
            bg_size = fg_image_shape
            fg_image2 = cv2.resize(fg_image2, bg_size)
        elif fg_image_shape < fg_image_shape2:
            bg_size = fg_image_shape2
            fg_image = cv2.resize(fg_image, bg_size)
            
        bg_image = self._generate_bg(bg_size, bg_color)
        ...
        b_mode = random.choice(BLEND_MODES)
        postprocess = self.postprocess.sample()
        
        image = _blend_images(
            fg_image, bg_image, blend_mode=b_mode, visibility_check=self.visibility_check
        )
        image = self._postprocess_image(image, postprocess)
        
        # if midground:
        #     fg_mask = _create_mask(fg_image2, self.foreground_mask_pad)
        #     mg_image, _ = self._generate_mg(mg_color, mg_style, fg_mask)
        #     bg_image = _blend_images(
        #         mg_image, bg_image, visibility_check=self.visibility_check
        #     )

        image2 = _blend_images(
            fg_image2, bg_image, blend_mode=b_mode, visibility_check=self.visibility_check
        )
        image2 = self._postprocess_image(image2, postprocess)
    ...
    def save(self, root, data, idx):
        ...
        shard = str(idx // 10000)
        image_key = os.path.join("images", shard, f"{idx}_0.jpg")
        ...
        shard = str(idx // 10000)
        image_key = os.path.join("images", shard, f"{idx}_1.jpg")
        image_path = os.path.join(root, image_key)
    ...
    def _generate_fg(self, color, style):
        shape = self.shape.sample()
        layout = self.layout.sample()
        ...
        self.shape.apply(char_layers, shape)
        self.layout.apply(char_layers, layout)
        
        # label2 = self.corpus.data(self.corpus.sample())
        label2 = next(text_iter)
        chars2 = utils.split_text(label2, reorder=True)
        text2 = "".join(chars2)

        char_layers2 = [layers.TextLayer(char, **font) for char in chars2]
        self.shape.apply(char_layers2, shape)
        self.layout.apply(char_layers2, layout)

        layer = layers.Group(char_layers).merge()
        layer2 = layers.Group(char_layers2).merge()
        
        self.color.apply([layer, layer2], color)
        self.texture.apply([layer, layer2], texture)
        self.style.apply([layer, layer2], style)
        self.transform.apply([layer, layer2], transform)
        self.fit.apply([layer, layer2], fit)
        self.pad.apply([layer, layer2], pad)
        out = layer.output()
        out2 = layer2.output()

        return out, out2, label, label2
    ...
    def _postprocess_image(self, image, postprocess):
        layer = layers.Layer(image)
        self.postprocess.apply([layer], postprocess)
        out = layer.output()
        return out
